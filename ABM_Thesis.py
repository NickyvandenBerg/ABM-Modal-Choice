...
class TravelAgent (mesa.Agent):
    def __init__(self, unique_id, model, origin_position, age, income, env_aware): 
        ...
        self.carbon_distanceticket_tax = False
    ...
    def path_train(self):
        if self.model.schedule.steps == 0: 
            if self.destination is not None: #checks if the self path list is empty
    ...
    def train_time_calculator(self):
        if self.model.schedule.steps == 0:
            trainspeed = self.pert_random(100, 120, 320) #km/h
            self.train_time = self.train_distance / trainspeed
            return self.train_time
    
    def air_time_calculator(self):
        if self.model.schedule.steps == 0:
            airspeed = self.pert_random(740, 780, 930) #km/h
    ...
    def air_price_calculator(self):
        if self.model.schedule.steps == 0:
            if self.air_distance < 1000:
                pricekm =self.pert_random(0.05, 0.12, 0.19) #€/passenger km
            else:
                pricekm =self.pert_random(0.05, 0.06, 0.19) #€/passenger km
    ...
    def train_price_calculator(self):
        if self.model.schedule.steps == 0:
            ...
            if self.train_distance < 1000:
                pricekm = self.pert_random(0.10, 0.15, 0.30) #€/passenger km
                self.train_price = self.train_distance * pricekm
            elif self.train_distance >= 1000:
                pricekm = self.pert_random(0.10, 0.12, 0.30) #€/passenger km
                self.train_price = self.train_distance * pricekm
    ...
    def carbon_tax(self):
        if self.model.schedule.steps != 0:
            return
        t_carbon_emission = 0.04466 #€/kg CO2
        
        "change fuel tax by blocking out the other values"
        t_carbon_fuel = 0.067       #€/kg CO2
        #t_carbon_fuel = 0.131      #€/kg CO2
        #t_carbon_fuel = 0.198      #€/kg CO2
        
        t_carbon_flatticket = 10.43 #€/ticket
        
        t_carbon_ticket_short = 25.30   #€/ticket
        t_carbon_ticket_long = 10.12    #€/ticket
    ...
    def calculate_train_score(self): #relative value * characteristic value
        if self.model.schedule.steps == 0:
            #relative values
...
class TravelModel(mesa.Model):
    def __init__(self):
        self.num_agents = df['agents'].sum() #number of agents equal to sum of agents per city
        ...
        with open('rail_network.data', 'rb') as file:
